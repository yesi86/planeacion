<div id="deleteModal-{{ $puesto->id }}" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 {{ $errors->any() ? 'block' : 'hidden' }}">
    <div class="bg-white rounded-lg p-6 w-96">
        <div class="bg-gray-100 px-4 py-1 border-b flex justify-between items-center rounded-b-xl">
            <h3 class="text-xl font-semibold">Eliminar: <span class="text-red-500">{{ $puesto['name'] }}</span></h3>
        </div>
        <form method="POST" action="{{ route('puestos.destroy', $puesto->id)}}" id="delete-form-{{ $puesto->id }}">
            ...
            <div class="flex justify-end space-x-2">
                ...
                <button type="button" class="confirm-delete-action bg-red-500 text-white py-2 px-4 rounded hover:bg-red-600" data-item-id="{{ $puesto->id }}">
                    Confirmar
                </button>
            </div>
        </form>
