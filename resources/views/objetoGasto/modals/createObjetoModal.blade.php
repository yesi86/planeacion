<div id="createObjetoModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 {{ request()->has('showModal') || $errors->any() ? 'block' : 'hidden' }}">
    <div class="bg-white rounded-xl shadow-lg max-w-lg w-full p-6">
        ...
        <form id="createObjetoForm" method="POST" action="{{ route('objeto.store') }}">
            @csrf
            ...
            <div class="mb-4">
                ...
                <div class="relative mt-1">
                    <select id="capitulo" name="capitulo" 
                        class="block w-full border border-gray rounded-lg shadow-sm py-2 px-3 text-sm focus:ring-green-500 focus:border-green-500 @error('capitulo') border-red-500 @enderror" 
                        required>
            ...
            <div class="mb-4">
                ...
                <div class="relative mt-1">
                    <input type="text" id="partida" name="partida" 
                        class="block w-full border border-gray rounded-lg shadow-sm py-2 px-3 text-sm focus:ring-green-500 focus:border-green-500 @error('partida') border-red-500 @enderror" 
                        value="{{ old('partida') }}" required placeholder="Escriba la partida del objeto por gasto">
                    @error('partida')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-4">
                ...
                <div class="relative mt-1">
                    <input type="text" id="descripcion" name="descripcion" 
                        class="block w-full border border-gray rounded-lg shadow-sm py-2 px-3 text-sm focus:ring-green-500 focus:border-green-500 @error('descripcion') border-red-500 @enderror" 
                        value="{{ old('descripcion') }}" required placeholder="Escriba la descripción del objeto por gasto">
                    @error('descripcion')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
