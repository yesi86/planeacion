<div id="deleteAccionModal-{{ $accion->id }}" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 {{ $errors->any() ? 'block' : 'hidden' }}">
    <div class="bg-white rounded-lg p-6 w-96">
        ...
        <div class="bg-gray-100 rounded-md p-3 text-center mb-4">
            <span class="block font-semibold text-gray-800">{{ $accion['Folio'] }}</span>
            <span class="block text-gray-600">{{ $accion['descripcion'] }}</span>
        </div>
        <form method="POST" action="{{ route('acciones.destroy', $accion->id) }}" id="delete-form-{{ $accion->id }}">
            @csrf
            @method('DELETE')
            <div class="flex justify-between space-x-4">
                ...
                <button type="button" class="confirm-delete-action bg-red-500 text-white py-2 px-4 w-full rounded hover:bg-red-600 focus:outline-none" data-item-id="{{ $accion->id }}">
                    Confirmar
                </button>
            </div>
        </form>
